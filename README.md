<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f0c29,50:302b63,100:24243e&height=200&section=header&text=Sujal%20Warghe&fontSize=52&fontColor=ffffff&fontAlignY=38&desc=Aspiring%20AI%20Engineer%20%7C%20Applied%20AI%20%26%20ML%20Developer&descAlignY=58&descSize=18&descColor=a0aec0" width="100%"/>

<br/>

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=22&pause=1000&color=7C83FD&center=true&vCenter=true&width=600&lines=Building+Real-World+AI+Systems;NLP+%7C+RAG+%7C+Computer+Vision+%7C+LLMs;End-to-End+Projects%2C+Not+Just+Demos;Turning+Ideas+into+Deployed+AI+Solutions)](https://git.io/typing-svg)

<br/>

<img src="https://komarev.com/ghpvc/?username=sujalwarghe&style=for-the-badge&color=7C83FD&label=PROFILE+VIEWS" alt="Profile Views"/>

</div>

---

## `$ whoami`

```python
engineer = {
    "name"     : "Sujal Warghe",
    "location" : "India 🇮🇳",
    "role"     : "Aspiring AI Engineer | Applied AI & ML Developer",
    "focus"    : ["LLM Applications", "RAG Systems", "Computer Vision", "NLP"],
    "mindset"  : "Project-driven. Hands-on. Builder-first.",
    "status"   : "Building real-world AI — one system at a time."
}
```

I build AI systems that solve actual problems. My approach is grounded in end-to-end development — from architecture decisions to deployment-ready code. I'm not here for theory alone; I'm here to ship things that work.

---

## ⚙️ Tech Stack & Skills

<div align="center">

### Languages
![Python](https://img.shields.io/badge/Python-0f0c29?style=for-the-badge&logo=python&logoColor=FFD43B)
![Java](https://img.shields.io/badge/Java-0f0c29?style=for-the-badge&logo=openjdk&logoColor=ED8B00)

### AI / ML
![scikit-learn](https://img.shields.io/badge/Machine%20Learning-0f0c29?style=for-the-badge&logo=scikitlearn&logoColor=F7931E)
![HuggingFace](https://img.shields.io/badge/Transformers%20%26%20NLP-0f0c29?style=for-the-badge&logo=huggingface&logoColor=FFD21E)
![LangChain](https://img.shields.io/badge/RAG%20Systems-0f0c29?style=for-the-badge&logo=chainlink&logoColor=7C83FD)
![OpenCV](https://img.shields.io/badge/Computer%20Vision-0f0c29?style=for-the-badge&logo=opencv&logoColor=5C3EE8)
![DeepFace](https://img.shields.io/badge/Face%20Recognition-0f0c29?style=for-the-badge&logo=ai&logoColor=FC3D21)

### Frameworks & Tools
![Flask](https://img.shields.io/badge/Flask-0f0c29?style=for-the-badge&logo=flask&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-0f0c29?style=for-the-badge&logo=streamlit&logoColor=FF4B4B)
![Git](https://img.shields.io/badge/Git-0f0c29?style=for-the-badge&logo=git&logoColor=F05032)
![REST API](https://img.shields.io/badge/REST%20APIs-0f0c29?style=for-the-badge&logo=fastapi&logoColor=009688)
![Browser Automation](https://img.shields.io/badge/Browser%20Automation-0f0c29?style=for-the-badge&logo=selenium&logoColor=43B02A)

</div>

---

## 🚀 Featured Projects

> Each project here is **end-to-end** — designed, built, structured, and documented with deployment in mind.

<table>
<tr>
<td width="50%" valign="top">

### 🎓 College Admission Chatbot
An AI-powered chatbot built for real college admission queries. Uses RAG architecture to retrieve accurate, context-aware answers from a structured knowledge base.

**Stack:** Python · RAG · NLP · Streamlit

</td>
<td width="50%" valign="top">

### 🧠 Face Intelligence System
Real-time face recognition pipeline with multi-attribute detection — age, gender, and emotion — using DeepFace and OpenCV.

**Stack:** Python · DeepFace · OpenCV · Flask

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 💊 Medical Recommendation System
ML-based system that suggests medical recommendations based on patient symptoms and history. Focused on practical usefulness over complexity.

**Stack:** Python · Scikit-learn · Flask

</td>
<td width="50%" valign="top">

### 🤖 AI Meme Generator
Automated meme generation pipeline using browser automation + LLM prompting. Combines creativity with engineering in a lightweight tool.

**Stack:** Python · Browser Automation · LLM APIs

</td>
</tr>
</table>

---

## 📜 Certifications

| Issuer | Certification |
|--------|--------------|
| 🟢 **NVIDIA** | Introduction to Transformer-Based NLP |
| 🔵 **Microsoft Learn** | Plan and Prepare AI Solutions on Azure |

---

## 🎯 Current Focus

```
▸ Deepening expertise in LLM-based application development
▸ Building production-grade RAG pipelines
▸ Exploring agentic AI workflows and tool-use frameworks
▸ Strengthening deployment practices (APIs, containers, cloud)
```

---

## 💡 Philosophy

> *"Don't just learn AI — build with it. A well-documented, deployed project teaches more than a hundred tutorials."*

I believe in **clean structure, honest documentation, and systems that actually work**. Every project I build is treated as a real product — with a proper README, reproducible setup, and clear purpose. Responsible and scalable AI isn't a goal for later; it's built into how I work now.

---

## 📊 GitHub Stats

<div align="center">

<img height="165em" src="https://github-readme-stats.vercel.app/api?username=sujalwarghe&show_icons=true&theme=midnight-purple&hide_border=true&bg_color=0f0c29&title_color=7C83FD&icon_color=7C83FD&text_color=ffffff&count_private=true"/>
<img height="165em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=sujalwarghe&layout=compact&theme=midnight-purple&hide_border=true&bg_color=0f0c29&title_color=7C83FD&text_color=ffffff"/>

<br/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=sujalwarghe&theme=midnight-purple&hide_border=true&background=0f0c29&ring=7C83FD&fire=FC3D21&currStreakLabel=7C83FD" alt="GitHub Streak"/>

</div>

---

## 🌐 Let's Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0f0c29?style=for-the-badge&logo=linkedin&logoColor=0A66C2)](https://linkedin.com/in/sujalwarghe)
[![GitHub](https://img.shields.io/badge/GitHub-0f0c29?style=for-the-badge&logo=github&logoColor=white)](https://github.com/sujalwarghe)
[![Email](https://img.shields.io/badge/Email-0f0c29?style=for-the-badge&logo=gmail&logoColor=EA4335)](mailto:sujalwarghe@email.com)

</div>

<br/>

<div align="center">

*If you're working on something meaningful in AI — a real problem, a thoughtful system, a hard challenge — I'm open to collaborating. Reach out.*

</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:24243e,50:302b63,100:0f0c29&height=120&section=footer" width="100%"/>
